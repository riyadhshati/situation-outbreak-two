...
class CTEPlayerAnimEvent_SO : public CBaseTempEntity
{
public:
	DECLARE_CLASS( CTEPlayerAnimEvent_SO, CBaseTempEntity );
	DECLARE_SERVERCLASS();

					CTEPlayerAnimEvent_SO( const char *name ) : CBaseTempEntity( name )
					{
					}
...
IMPLEMENT_SERVERCLASS_ST_NOBASE( CTEPlayerAnimEvent_SO, DT_TEPlayerAnimEvent_SO )
	SendPropEHandle( SENDINFO( m_hPlayer ) ),
	SendPropInt( SENDINFO( m_iEvent ), Q_log2( PLAYERANIMEVENT_COUNT ) + 1, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_nData ), 32 )
END_SEND_TABLE()

static CTEPlayerAnimEvent_SO g_TEPlayerAnimEvent_SO( "PlayerAnimEvent" );

void TE_PlayerAnimEvent_SO( CBasePlayer *pPlayer, PlayerAnimEvent_t event, int nData )
{
	...
	g_TEPlayerAnimEvent_SO.m_hPlayer = pPlayer;
	g_TEPlayerAnimEvent_SO.m_iEvent = event;
	g_TEPlayerAnimEvent_SO.m_nData = nData;
	g_TEPlayerAnimEvent_SO.Create( filter, 0 );
}

void CHL2MP_Player::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	m_PlayerAnimState->DoAnimationEvent( event, nData );
	TE_PlayerAnimEvent_SO( this, event, nData );	// Send to any clients who can see this guy.
}
