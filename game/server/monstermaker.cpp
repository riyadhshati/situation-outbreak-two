...
//////
	...
	// SO2 - James
	// Add a custom spawner entity for zombie NPCs
...
LINK_ENTITY_TO_CLASS( so_zombie_spawner, CZombieSpawner );

BEGIN_DATADESC( CZombieSpawner )

	DEFINE_KEYFIELD( m_iszNPCClassname,			FIELD_STRING,	"NPCType" ),
	DEFINE_KEYFIELD( m_bShouldRandomize,		FIELD_BOOLEAN,	"NPCTypeRandom" ),
	DEFINE_KEYFIELD( m_ChildTargetName,			FIELD_STRING,	"NPCTargetname" ),
	DEFINE_KEYFIELD( m_SquadName,				FIELD_STRING,	"NPCSquadName" ),
	DEFINE_KEYFIELD( m_strHintGroup,			FIELD_STRING,	"NPCHintGroup" ),
...
void CZombieSpawner::Precache( void )
{
	...
	for ( int i = 0; i < ARRAYSIZE(ZombieNPCs); i++ )
		UTIL_PrecacheOther( ZombieNPCs[i] );
}
...
void CZombieSpawner::Spawn( void )
{
	...
	// Add a custom rally point entity for NPC spawners
	pRallyEnt = dynamic_cast<CNPCRallyPoint*>( gEntList.FindEntityByName( NULL, rallypointName, this ) );
}
...
void CZombieSpawner::MakeNPC( void )
{
	...
	// By default, use the class name of the zombie NPC the mapper specifies
	const char *NPCClassName = STRING(m_iszNPCClassname);

	// If the mapper wants to randomize the zombie NPC chosen, do that instead
	if ( m_bShouldRandomize )
		NPCClassName = ZombieNPCs[RandomInt(0, ARRAYSIZE(ZombieNPCs) - 1)];

	// Ensure the NPC type we've defined is proper
	CNPC_SO_BaseZombie *pent = dynamic_cast<CNPC_SO_BaseZombie*>( CreateEntityByName( NPCClassName ) );
	if ( !pent )
	{
		Warning("-----\n");
		Warning("WARNING: Zombie spawner defined an invalid NPC type!\n");
		Warning("An so_zombie_spawner entity [%s] attempted to spawn an invalid type of NPC.\n", STRING(GetEntityName()));
		Warning("The invalid NPC has not been spawned (for obvious reasons).\n");
		Warning("-----\n");
		return;
	}
