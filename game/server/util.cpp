...
//////
	...
	// Based on the same function from a previous version of the Source engine
...
CBasePlayer *UTIL_GetLocalPlayer( void )
{
	// Return any old player since we don't have an origin to work with
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CBasePlayer *pPlayer = UTIL_PlayerByIndex( i );
		if ( pPlayer )
	...
	// No players online, so unfortunately we can't do anything but return NULL here, sorry!
	return NULL;
}
...
CBasePlayer *UTIL_GetNearestPlayer( const Vector &origin )
{
	...
	// If something somehow goes wrong and we didn't get a player from that first check, return any old player
	if ( !pNearestPlayer )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			CBasePlayer *pPlayer = UTIL_PlayerByIndex( i );
			if ( pPlayer && (pPlayer->GetTeamNumber() != TEAM_SPECTATOR) && pPlayer->IsAlive() )	// still don't count those who aren't playing or are dead
				return pPlayer;
		}
	}

	// This could be a serious problem, but there's really nothing we can do but return NULL here...
	return NULL;
}
